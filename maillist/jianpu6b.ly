     ...
     (make-engraver
      ...
      (listeners
       ((note-event engraver event)
        (set! dot-count (ly:duration-dot-count (ly:event-property event 'duration)))))
       ...
       ((note-head-interface engraver grob source-engraver)
        ...
        (if (ly:grob-property-data grob 'stencil)
            (let* ((staff-context (ly:translator-context engraver))
                   ...
                   ;; add duration dashes to right of half note and whole note and
                   ;; handle dot count on dotted half notes and whole notes
                   (dur-log (ly:grob-property grob 'duration-log))
                   (stencil-b
                    (case dur-log
                      ;; whole notes
                      ((0)
                       (case dot-count
                         ...
                         ((2) (begin (set! new-dot-count 0)
                                (add-dash-right (add-dash-right (add-dash-right (add-dash-right
                                                                                 (add-dash-right (add-dash-right stencil-a))))))))
                         ;; single dotted whole note
                         ((1) (begin (set! new-dot-count 0)
                                (add-dash-right (add-dash-right (add-dash-right (add-dash-right
                                                                                 (add-dash-right stencil-a)))))))
                         ;; undotted whole note
                         (else (begin (set! new-dot-count #f)
                                 (add-dash-right (add-dash-right (add-dash-right stencil-a)))))
                         ))
                      ...
                      ((1)
                       (case dot-count
                         ...
                         ((2) (begin (set! new-dot-count 1)
                                (add-dash-right (add-dash-right stencil-a))))
                         ;; single dotted half note
                         ((1) (begin (set! new-dot-count 0)
                                (add-dash-right (add-dash-right stencil-a))))
                         ;; undotted half note
                         (else (begin (set! new-dot-count #f)
                   ...
                   (stencil-d (case octave
                                ...
                                (else stencil-c))) )
              ;; (display dur-log)(newline)(newline)
              ;; set the stencil for the note head grob
              (ly:grob-set-property! grob 'stencil stencil-d)
              )))

       ;; another acknowledger for dots on dotted half notes and whole notes
       ((dots-interface engraver grob source-engraver)
        (if new-dot-count (ly:grob-set-property! grob 'dot-count new-dot-count)))
       ))))
